...
import TokenArtifact from "./contracts/SystICO.json";

import contractAddress from "./contracts/contract-address.json";
import timeInterval from "./contracts/time-interval.json";

const provider = new ethers.providers.Web3Provider(window.ethereum);
const systICO = new ethers.Contract(
  contractAddress.SystICO,
  TokenArtifact.abi,
  provider.getSigner(0)
);

async function getVariables() {
  const ra = await systICO.raisedAmount();
  const hc = await systICO.hardCap();
  console.log(ra, hc);
}
...
const timeField = (startTime, endTime, flag) => {
  ...
  const disabled =
    currentTime < endTime || (flag && systICO.raisedAmount() < 1e25);
...
export default function RecipeReviewCard() {
  ...
  const progress = values.connected
    ? (100 * systICO.raisedAmount()) / systICO.hardCap()
    : 0;

  const deposit = async (value) => {
    try {
      await systICO.deposit({ value });
    } catch (error) {
